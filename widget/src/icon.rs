...
use iced::{Color, ContentFit, Element, Length, Point, Rectangle, Rotation, Size, Theme, Vector};
...
pub struct Icon<'a, Theme = iced::Theme>
where
    Theme: Catalog,
{
    handle: Handle,
...
impl<Message, Theme, Renderer> Widget<Message, Theme, Renderer> for Icon<'_, Theme>
where
    ...
    Theme: Catalog,
{
    ...
    fn draw(
        ...
        let custom_style = theme.style(&self.class, status);

        let render = |renderer: &mut Renderer| {
            renderer.draw_svg(
                svg::Svg {
                    handle: self.handle.clone(),
                    color: Some(custom_style.color.unwrap_or(style.text_color)),
...
{
    fn from(icon: Icon<'a, Theme>) -> Element<'a, Message, Theme, Renderer> {
        Element::new(icon)
    }
}
